...
public class first_negative_in_every_window_size_k {
    ...
    static List<Integer> firstNegInt(int arr[], int k) {
        ...
        for(int i=0;i<k;i++){
            if(arr[i]<0) qu.add(arr[i]);
        }
        
        List<Integer> result = new ArrayList<>();
        
        if(qu.size()==0) result.add(0);
        else result.add(qu.getFirst());
        ...
        for(int i=k;i<arr.length;i++){
            ...
            if(qu.size()!=0){
            int first = qu.getFirst();
            
            if(first==arr[i-k]) qu.pop();
            if(qu.size()==0) result.add(0);
            else result.add(qu.getFirst());
